...
- **Lesson:** `filter: blur()` samples beyond the element's box, so a background pinned at `inset: 0` shows a soft, faded border where the blur runs out of pixels. Extend the layer past the edges (`inset: -30px` or so) and let the section's `overflow: hidden` clip it, so the blur stays edge-to-edge. Darken in the same `filter` with `brightness()` (cheap, GPU-composited) rather than stacking another overlay. Keep any element that must stay sharp (decorative SVG lines, glow) as a SEPARATE sibling layer at a higher z-index, not a child of the blurred element, since `filter` applies to descendants too. Its own CSS-background glow also survives because it lives on that separate layer. PITFALL: once the base background is darkened to near-uniform, dark gradient overlays that were previously invisible against a textured image (corner "suppressors", heavy multi-stop bottom fades, and the hard clip edge of a `-32px`-extended blurred layer) suddenly read as ugly black bands/blobs. When darkening a background, audit and remove now-redundant dark overlays, and fade the blurred layer itself with a `mask-image` (e.g. `linear-gradient(to bottom, #000 58%, transparent 90%)`, and only extend the non-faded sides) so it dissolves into the section color instead of clipping into a seam, rather than stacking another opaque gradient on top.
- **Source:** success · 2026-07-21 · hero artwork blurred+darkened; removed suppressor + heavy fade bands, masked the bg bottom instead

### De-slop a card's STYLE without touching its animations
- **When:** A user likes an interaction/animation but says the card/section "looks AI generated" and wants the visual style fixed (e.g. the Hero product stack cards in `Hero.jsx` + `.hero-product-*` in `index.css`).
- **Lesson:** Keep every animated class name, keyframe, and stagger delay intact; change only visual tokens (color, border, type, nesting, decoration) so the motion is unaffected. The recurring AI-card tells to hunt and remove: (1) tiny UPPERCASE letter-spaced eyebrow labels, switch to sentence case, normal tracking, larger and higher-contrast so it reads as a real title (the JSX copy is usually already sentence case, the uppercase is pure CSS `text-transform`); (2) a glowing status dot (`box-shadow: 0 0 0 5px rgba(green,.06)` pulse ring), drop the ring, keep a plain small dot; (3) outlined "MATCHED"/"Live" status PILLS, strip the border/bg/radius and make it quiet muted text so the label leads; (4) card-in-a-card nesting (a bordered rounded scene box holding another bordered rounded row), flatten the OUTER wrapper to open padding+gap so only ONE bordered element remains; (5) neon-green outline around the whole card + green hover glow, make the frame a neutral hairline and RESERVE the green for meaning (a solid-green avatar chip with dark text, the status dot, an arrow text-link) instead of sprinkling green glows everywhere; (6) faint low-contrast body copy (~.64 alpha), bump toward the ink end (~.8) and up a hair in size; (7) tinted-green pill CTA, convert to a text link with an arrow that translateX on hover. Net move: commit color to a few meaningful accents, flatten nesting, sharpen contrast and typography. Verify with `npm run build` from `website/`.
- **Source:** success · 2026-07-21 · hero product cards de-slopped, animations untouched
- **Follow-up (removing a card's header/topline row):** A card-header strip ("label · Matched" + divider) is itself a common SaaS-card cliché; removing it declutters since the big headline is redundant with it. When you delete a fixed-height header row from a fixed-height, absolutely-positioned card whose content is a CSS grid, you MUST (1) drop that row from `grid-template-rows` at EVERY breakpoint (there was a second `@media 1024-1199` override with its own `46px ...` track), and don't just hand the freed height to the trailing `1fr` row or the card looks bottom-heavy/empty, redistribute it to the visual/scene track; (2) remove the deleted element from any grouped entrance-animation selector list and its `animation-delay` line so no orphaned selector/dead delay remains; (3) leave the now-unused data fields (label/status) in the JSX array, harmless, rather than churning the data model. Rebuild to confirm.

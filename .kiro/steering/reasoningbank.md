...
- **Lesson:** Track pointer coordinates on a stable parent, keep real controls separate, and use an ambient layer plus a cursor-clipped spotlight layer. The ambient copy should remain visible enough to advertise the interaction, but reduce panel fills, shadows, and child opacity so borders and connector lines dominate. The spotlight copy can restore brightness and detail locally. Avoid a centered logo or status marker when the headline already owns the center, since it competes with reading hierarchy. Verify both the resting outlines and hover reveal in the rendered target viewport.
- **Source:** success · 2026-07-17 · ambient outline balance and center-marker removal